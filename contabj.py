...
def d2C(d):
    return Contact(d['name'], d['ref'], d['email'], d['phone1'], d['phone2'])

def C2d(C):
    return {'name' : C.name, 'ref' : C.ref, 'email' : C.email, 'phone1' : C.phone1, 'phone2' : C.phone2}
...
f = open(abfile, 'w')
j.dump(c, f, default = C2d) # Save the Contacts to File
f.close()